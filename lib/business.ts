...
export async function getActiveBusiness(client?: ServerSupabaseClient): Promise<ActiveBusiness> {
  ...
  const tenantSlug = requestHeaders.get("x-imerscloud-tenant-slug");

  if (!tenantSlug) redirect("/");

  if (tenantSlug) {
    const { data: tenant, error: tenantError } = await supabase
      .from("businesses")
      .select("id,name,slug,logo_url,timezone,whatsapp,sku_prefix,deleted_at")
      .eq("slug", tenantSlug)
      .is("deleted_at", null)
      .eq("is_active", true)
      .not("tenant_status", "in", "(expired,suspended)")
      .maybeSingle();
    if (tenantError) throw new Error(`TENANT_LOOKUP_FAILED: ${tenantError.message}`);
    if (!tenant) redirect(`/${tenantSlug}`);
    const { data: authState } = await supabase.auth.getUser();
    if (!authState.user) redirect(`/${tenantSlug}/login`);
    const { data: membership, error: membershipError } = await supabase
      ...
      .eq("status", "active")
      .maybeSingle();
    if (membershipError) throw new Error(`TENANT_MEMBERSHIP_LOOKUP_FAILED: ${membershipError.message}`);
    if (!membership) redirect(`/${tenantSlug}`);
    return {
      id: tenant.id, name: tenant.name, slug: tenant.slug, logo_url: tenant.logo_url ?? null,
      timezone: tenant.timezone || "Asia/Jakarta", whatsapp: tenant.whatsapp ?? null,
      sku_prefix: tenant.sku_prefix || "SKU", role: membership.role as ActiveBusiness["role"],
    };
  }
