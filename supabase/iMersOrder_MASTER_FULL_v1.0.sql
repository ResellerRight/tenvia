...
begin
  ...
  where is_active=true and deleted_at is null
    and (
      (p_payload ? '_cloud_business_id' and id=(p_payload->>'_cloud_business_id')::uuid)
      or not (p_payload ? '_cloud_business_id')
    )
  order by created_at limit 1;
  if not found then raise exception 'Business not found'; end if;
...
-- iMersOrders Cloud r31: registration toggle, first-user Super Admin bootstrap, and tenant owner invitations.
